...
install_nvim_deps() {
  ...
  # Lombok agent for jdtls: without it, jdtls can't see members generated by
  # @Slf4j / @Data / @Builder etc. (the `log` field shows as an error). jdtls.lua
  # loads it as a javaagent when this jar exists.
  local lombok="${HOME}/.local/share/nvim/lombok.jar"
  if [ -f "${lombok}" ]; then
    log "lombok: already present"
  else
    log "lombok: downloading -> ${lombok}..."
    mkdir -p "$(dirname "${lombok}")"
    curl -fsSL -o "${lombok}" https://projectlombok.org/downloads/lombok.jar
  fi
}
